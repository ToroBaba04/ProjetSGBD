...
class OpenCartMonitor(tk.Tk):
    ...
    def populate_dashboard(self):
        # Exemples de graphiques de démonstration
        fig1, ax1 = plt.subplots()
        ax1.bar(["Product A", "Product B", "Product C"], [20, 15, 10])
        ax1.set_title("Sales by Product")
        ax1.set_xlabel("Product")
        ax1.set_ylabel("Sales")

        fig2, ax2 = plt.subplots()
        ax2.pie([25, 35, 40], labels=["In Stock", "Low Stock", "Out of Stock"], autopct='%1.1f%%')
        ax2.set_title("Stock Status")

        # Intégrer les graphiques dans la fenêtre Tkinter
